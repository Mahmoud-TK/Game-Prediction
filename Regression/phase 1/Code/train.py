...
def pre_processing(X, y, xtest, y_test):
    ...
    with open('models/encoder_lists.pkl', 'wb') as file:
        pickle.dump(encoders, file)
    ...
    with open('models/Scaling.pkl', 'wb') as file:
        pickle.dump(savingScale, file)

    with open('models/SCsize.pkl', 'wb') as file:
        pickle.dump(saveSizeSale, file)

    y, y_test,saveNormalize = normalizedY(y, y_test)

    with open('models/normalizeY.pkl', 'wb') as file:
    ...
    with open('models/selection.pkl', 'wb') as file:
...
def polynomial_regression(X_train, y_train, X_test, degree):
    ...
    with open('models/polyFeatures.pkl', 'wb') as file:
...
def multiple_regression(X, y, X_test):
    ...
    with open('models/multiple_regression_model.pkl', 'wb') as file:
...
def svr(X_train, Y_train, X_test):
    ...
    with open('models/support_vector_regressor_model.pkl', 'wb') as file:
...
# read data
dataset = pd.read_csv("datasets/games-regression-dataset.csv")
X = dataset.iloc[:, 0:-1]

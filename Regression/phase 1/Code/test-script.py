...
dataset = pd.read_csv("test-regression.csv")
...
X_test['Age Rating'] = X_test['Age Rating'].str.replace('+', '', regex=False).astype(int)
...
#--------------------------------------encoding lissts
encoders = {}
with open('encoder_lists.pkl', 'rb') as file:
    encoders = pickle.load(file)
...
for c in list_cols:
    ...
    X_test[c] = X_test[c].apply(lambda x: [encoded_dict[lang] if lang in encoded_dict else 0 for lang in x])
...
with open('SCsize.pkl', 'rb') as file:
...
with open('normalizeY.pkl', 'rb') as file:
...
#-------------------------------------------------selection
with open('selection.pkl', 'rb') as file:
...
with open('polyFeatures.pkl', 'rb') as file:
...
filename = 'support_vector_regressor_model.pkl'
loaded_model = pickle.load(open(filename, 'rb'))
pred = loaded_model.predict(X_test)
